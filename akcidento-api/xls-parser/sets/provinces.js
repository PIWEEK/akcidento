...
// Get the XLSX tab
const provincesWb = workbook.Sheets['ATR-D.2.1'];
...
// const rangeByProvince = XLSX.utils.decode_range(provinces['!ref']);
const parseProvincesColumns = (start, end) => {
    let provincesArray = []
    for (let rowNum = start; rowNum <= end; rowNum++) {
        const provinceName = provincesWb[XLSX.utils.encode_cell({r: rowNum, c: 0})];
        provincesArray.push(provinceName.v.trim());
    }
...
const provinces = {
    ...
    murcia: ['murcia'],
    navarra: ['navarra'],
    euskalHerria: parseProvincesColumns(80, 83),
    rioja: ['la rioja'],
    ceuta: ['ceuta'],
    melilla: ['melilla']
}
...
// provinces.map(async (sector, index) => {
//     let sd = await sectorController.createSector(sector);
//     return sd;
// });
const generateProvince = (region, provinces) => {
